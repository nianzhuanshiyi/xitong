...
export function EuropeIdeaDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`/api/europe-ideas/${id}`);
  ...
  const handleDelete = async () => {
    ...
    const r = await fetch(`/api/europe-ideas/${id}`, { method: "DELETE" });
    if (r.ok) { toast.success("已删除"); router.push("/dashboard/europe-ideas"); }
  ...
  const submitComment = async () => {
    ...
    setCommentBusy(true);
    try {
      const r = await fetch(`/api/europe-ideas/${id}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-1.5">
                  {data.keyFeatures.map((f, i) => (
                    <span key={i} className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700">{f}</span>
                  ))}
                </div>
              </div>
              <div>
