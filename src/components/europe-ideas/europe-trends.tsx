...
const SOURCE_LABELS: Record<string, string> = {
  ...
  amazon_bestseller: "Amazon Best Seller",
};

export function EuropeTrends() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/europe-ideas/trends?${params}`);
  ...
  const markets = filterMarket ? [filterMarket] : ["DE", "UK", "FR", "IT", "ES"];

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/europe-ideas" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
          <ArrowLeft className="size-4" /> 返回
        </Link>
        <h2 className="text-lg font-semibold">欧洲蓝海趋势</h2>
        <div className="ml-auto flex flex-wrap gap-2">
          {["", "DE", "UK", "FR", "IT", "ES"].map((m) => (
            <button
              key={m}
              className={cn(
            ...
            >
              {m ? `${MARKET_LABELS[m]?.flag ?? ""} ${MARKET_LABELS[m]?.label ?? m}` : "全部"}
            </button>
          ))}
        </div>
      </div>
                      ...
                      <CardContent className="p-3">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {t.keywords.slice(0, 4).map((kw, i) => (
                            <span key={i} className="rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] text-indigo-700">{kw}</span>
                          ))}
                        </div>
