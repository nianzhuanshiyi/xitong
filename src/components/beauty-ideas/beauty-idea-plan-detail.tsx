...
export function BeautyIdeaPlanDetail({ planId }: { planId: string }) {
  ...
  const loadPlan = useCallback(async () => {
    try {
      const r = await fetch(`/api/beauty-ideas/plans/${planId}`);
  ...
  return (
    <div className="space-y-6">
      {/* Back */}
      <Link href="/dashboard/beauty-ideas" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700">
        <ArrowLeft className="size-4" /> 返回美妆新品创意
      </Link>

      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 via-pink-50/50 to-fuchsia-50/30 p-6 shadow-sm">
            ...
            <div className="space-y-1.5">
              {keywords.map((kw, i) => (
                ...
                >
                  ...
                  <span className={cn("text-center", kw.sdr !== null && kw.sdr >= 0.5 ? "text-emerald-700 font-semibold" : kw.sdr !== null ? "text-red-600" : "text-slate-400")}>
      ...
      {activeTab === "market" && (
        <Card>
          <CardHeader><CardTitle className="text-base">市场分析</CardTitle></CardHeader>
          <CardContent className="space-y-6">
            ...
            {competitors.length > 0 && (
              <div className="border-t pt-4">
                <p className="text-xs font-semibold text-slate-500 mb-3">
                  卖家精灵筛选竞品（价格$15-40 / 月销≥300 / 月收入≥$5,000 / 评论50-2,000）
                </p>
                  ...
                  {competitors.map((p, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-3 rounded-lg border border-slate-100 p-3 text-xs">
                        <span className="shrink-0 size-6 rounded-full bg-rose-100 text-rose-600 font-bold flex items-center justify-center text-[11px]">
                          {i + 1}
                        </span>
      ...
      {activeTab === "execution" && (
        <div className="space-y-4">
          ...
          {plan.listingPlan && (
            <Card>
              <CardHeader><CardTitle className="text-sm">Listing 策略</CardTitle></CardHeader>
                ...
                <div className="space-y-3">
                  {timeline.map((item, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="shrink-0 size-6 rounded-full bg-rose-100 flex items-center justify-center text-[11px] font-bold text-rose-600">
                        {i + 1}
                      </div>
                      <div>
