...
export function ThreeCIdeaPlanDetail({ planId }: { planId: string }) {
  ...
  const loadPlan = useCallback(async () => {
    try {
      const r = await fetch(`/api/three-c-ideas/plans/${planId}`);
  ...
  if (!plan) {
    return (
      <div className="py-20 text-center">
        ...
        <Link href="/dashboard/three-c-ideas">
  ...
  return (
    <div className="space-y-6">
      {/* Back */}
      <Link href="/dashboard/three-c-ideas" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700">
        <ArrowLeft className="size-4" /> 返回 3C 新品创意
      </Link>

      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 via-indigo-50/50 to-violet-50/30 p-6 shadow-sm">
            ...
            <div className="space-y-1.5">
              {keywords.map((kw, i) => (
                ...
                >
                  ...
                  <span className={cn("text-center", kw.sdr !== null && kw.sdr >= 0.3 ? "text-emerald-700 font-semibold" : kw.sdr !== null ? "text-red-600" : "text-slate-400")}>
            ...
            {competitors.length > 0 && (
              <div className="border-t pt-4">
                <p className="text-xs font-semibold text-slate-500 mb-3">
                  卖家精灵筛选竞品（价格$20-100 / 月销≥200 / 评论≤3,000）
                </p>
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-4">
              {[
                ...
                { label: "成本", value: plan.estimatedCogs },
