  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="h-2 flex-1 rounded-full bg-slate-100">
        <div className="h-full rounded-full bg-cyan-400" style={{ width: `${(value / 25) * 100}%` }} />
      </div>
...
export function ThreeCIdeaDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`/api/3c-ideas/${id}`);
  ...
  const handleDelete = async () => {
    ...
    const r = await fetch(`/api/3c-ideas/${id}`, { method: "DELETE" });
    if (r.ok) { toast.success("已删除"); router.push("/dashboard/3c-ideas"); }
  ...
  const submitComment = async () => {
    ...
    setCommentBusy(true);
    try {
      const r = await fetch(`/api/3c-ideas/${id}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <h4 className="text-sm font-medium text-slate-700">核心功能</h4>
                <div className="mt-1 flex flex-wrap gap-1.5">
                  {data.keyFeatures.map((f, i) => (
                    <span key={i} className="rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-medium text-cyan-700">{f}</span>
                  ))}
                </div>
              </div>
              <div>
