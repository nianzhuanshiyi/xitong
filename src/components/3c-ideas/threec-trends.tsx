...
export function ThreeCTrends() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/3c-ideas/trends?${params}`);
  ...
  const markets = filterMarket ? [filterMarket] : ["US", "EU", "JP"];

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/3c-ideas" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
          <ArrowLeft className="size-4" /> 返回
        </Link>
        <h2 className="text-lg font-semibold">3C配件趋势</h2>
          ...
          {["", "US", "EU", "JP"].map((m) => (
            <button
              ...
              className={cn(
                ...
                filterMarket === m ? "bg-cyan-100 text-cyan-800" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              )}
                    ...
                    <Card key={t.id}>
                      <CardContent className="p-3">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {t.keywords.slice(0, 4).map((kw, i) => (
                            <span key={i} className="rounded-full bg-cyan-50 px-2 py-0.5 text-[10px] text-cyan-700">{kw}</span>
                          ))}
                        </div>
                        <div className="mt-2 flex items-center gap-2 text-[10px] text-slate-400">
                          ...
                          {t.ideaCount > 0 && (
                            <span className="rounded bg-cyan-50 px-1.5 py-0.5 text-cyan-600">{t.ideaCount} 个创意</span>
                          )}
                          {t.sourceUrl && (
                            <a href={t.sourceUrl} target="_blank" rel="noopener noreferrer" className="ml-auto text-cyan-500 hover:underline">
