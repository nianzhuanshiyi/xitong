...
export async function POST() {
  ...
  try {
    console.info("[beauty-scan] Step1: keyword_research 搜索蓝海关键词...");
    ...
    if (kwRes.ok && extractKwItems(kwRes.data).length < 5) {
      console.info("[beauty-scan] < 5 results, relaxing filters...");
      kwRes = await mcp.callToolSafe("keyword_research", {
        request: { marketplace: "US", departments: ["beauty"], minSearches: 500, maxProducts: 500, minSupplyDemandRatio: 2, maxRatings: 1000, size: 20, order: { field: "searches_growth", desc: true } },
      });
    }
    ...
    console.info(`[beauty-scan] Step1 完成: ${kwItems.length} 条`);

    console.info("[beauty-scan] Step2: Google Trends 验证...");
    const enriched = await enrichWithGoogleTrends(kwItems, "US", mcp, "[beauty-scan]");
    console.info(`[beauty-scan] Step2 完成: ${enriched.length} 条通过验证`);
