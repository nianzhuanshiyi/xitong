...
const SYSTEM_PROMPT = `你是一位资深3C电子产品经理，服务于亚马逊跨境3C卖家。
我们在深圳有成熟的3C配件供应链，产品主要在亚马逊线上销售。
    ...
    for (const idea of ideas) {
      ...
      if (idea.searchKeywords?.[0]) {
        try {
          ...
          console.warn("[3c-generate] MCP keyword_research failed");
        }
      }
      ...
      if (idea.searchKeywords?.[0]) {
        try {
          ...
          console.warn("[3c-generate] MCP product_research failed");
        }
      }
